...
def add_to_map():
    ...
    for direction in exits:
        if player.currentRoom.id not in maze_map:
            maze_map[player.currentRoom.id] = {f'{direction}': "?"}
        elif direction not in maze_map[player.currentRoom.id]:
            maze_map[player.currentRoom.id][direction] = "?"


def update_map(old_room, new_room, direction_traveled):
    maze_map[old_room][direction_traveled] = new_room
    maze_map[new_room][reverse_path(direction_traveled)] = old_room
    # print(maze_map)


def move(old_room):
    # print("room number", player.currentRoom.id)
    # if position not in maze_map:
    #     add_to_map()
    if player.currentRoom.n_to is not None:
        # print("north to",player.currentRoom.n_to.id)
        if player.currentRoom.n_to.id not in maze_map:
            player.travel('n')
            add_to_map()
            update_map(old_room, player.currentRoom.id, 'n')
            return 'n'
        elif "n" in maze_map[player.currentRoom.n_to.id]:
            if maze_map[player.currentRoom.id]['n'] == '?':
                player.travel('n')
                update_map(old_room, player.currentRoom.id, 'n')
                return 'n'
    if player.currentRoom.w_to is not None:
        # print("west to",player.currentRoom.w_to.id)
        if player.currentRoom.w_to.id not in maze_map:
            player.travel('w')
            add_to_map()
            update_map(old_room, player.currentRoom.id, 'w')
            return 'w'
        elif "w" in maze_map[player.currentRoom.w_to.id]:
            if maze_map[player.currentRoom.id]['w'] == '?':
                player.travel('w')
                update_map(old_room, player.currentRoom.id, 'w')
                return 'w'
    if player.currentRoom.e_to is not None:
        # print("east to",player.currentRoom.e_to.id)
        if player.currentRoom.e_to.id not in maze_map:
            player.travel('e')
            add_to_map()
            update_map(old_room, player.currentRoom.id, 'e')
            return 'e'
        elif "e" in maze_map[player.currentRoom.e_to.id]:
            if maze_map[player.currentRoom.id]['e'] == '?':
                player.travel('e')
                update_map(old_room, player.currentRoom.id, 'e')
                return 'e'
    if player.currentRoom.s_to is not None:
        # print("south to",player.currentRoom.s_to.id)
        if player.currentRoom.s_to.id not in maze_map:
            player.travel('s')
            add_to_map()
            update_map(old_room, player.currentRoom.id, 's')
            return 's'
        elif "s" in maze_map[player.currentRoom.s_to.id]:
            if maze_map[player.currentRoom.id]['s'] == '?':
                player.travel('s')
                update_map(old_room, player.currentRoom.id, 's')
                return 's'
    else:
...
def traverse_maze(starting_room):
    unique_rooms = 1
    while len(maze_map) != len(roomGraph):
        add_to_map()
        old_room = player.currentRoom.id
        next_move = move(old_room)
        ...
        if next_move is not None:
            ...
            last_step = steps.pop()
            # print(">>>>>>>>", steps)
            move_back = reverse_path(last_step)
...
traverse_maze(player.currentRoom.id)
